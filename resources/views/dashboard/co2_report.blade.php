...
@section('title', 'All Co2 Report')

@section('content')
    <form class="form-inline mb-5 col-lg-12" method="GET" action="{{ route('home.checksheet.co2') }}">
            ...
            <div class="card-body">
                <div class="table-responsive">
                    <table class="table table-striped table-sm col-lg-12" id="dtBasicExample" style="width: 100%">
                        ...
                        <tbody>
                            @foreach ($co2Data as $co2)
                                <tr>
                                    <td>{{ $loop->iteration }}</td>
                                    <td>{{ $co2['tabung_number'] }}</td>
                                    <td>{{ $co2['location_name'] }}</td>
                                    <td>{{ $co2['plant'] }}</td>
                                    @for ($month = 1; $month <= 12; $month++)
                                        <td>
                                            @if (isset($co2['months'][$month]))
                                                @if (in_array('OK', $co2['months'][$month]))
                                                    <span class="badge bg-success">OK</span>
                                                @else
                                                    @php
                                                        $issueCodes = implode('+', $co2['months'][$month]);
                                                    @endphp
    ...
    <div class="card rounded-top-0 mt-0 col-lg-12">
        <div class="card-body">
            ...
            <table class="table table-sm table-borderless">
                <tbody>
                    <tr>
                        <td scope="col">1. Cover</td>
                        <td scope="col">= a</td>
                        <td scope="col">3. Lock Pin</td>
                        <td scope="col">= c</td>
                        <td scope="col">5. Kebocoran Regulator Tabung</td>
                        <td scope="col">= e</td>
                    </tr>
                    <tr>
                        <td scope="col">2. Tabung</td>
                        <td scope="col">= b</td>
                        <td scope="col">4. Segel Lock Pin</td>
                        <td scope="col">= d</td>
                        <td scope="col">6. Selang</td>
                        <td scope="col">= f</td>
                    </tr>
                </tbody>
            </table>
