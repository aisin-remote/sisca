...
@section('title', 'All Tandu Report')

@section('content')
    <form class="form-inline mb-5 col-lg-12" method="GET" action="{{ route('home.checksheet.tandu') }}">
        ...
        <div class="card-body">
            <div class="table-responsive">
                <table class="table table-striped table-sm col-lg-12" id="dtBasicExample" style="width: 100%">
                    ...
                    <tbody>
                        @foreach ($tanduData as $tandu)
                            <tr>
                                <td>{{ $loop->iteration }}</td>
                                <td>{{ $tandu['tandu_number'] }}</td>
                                <td>{{ $tandu['location_name'] }}</td>
                                @for ($month = 1; $month <= 12; $month++)
                                    <td>
                                        @if (isset($tandu['months'][$month]))
                                            @if (in_array('OK', $tandu['months'][$month]))
                                                <span class="badge bg-success">OK</span>
                                            @else
                                                @php
                                                    $issueCodes = implode('+', $tandu['months'][$month]);
                                                @endphp
        ...
        <div class="card-body">
            ...
            <div class="table-responsive">
                <table class="table table-sm table-borderless">
                    <tbody>
                        <tr>
                            <td scope="col">1. Kunci Pintu</td>
                            <td scope="col">= a</td>
                            <td scope="col">4. Hand Grip</td>
                            <td scope="col">= d</td>
                            <td scope="col">7. Kaki</td>
                            <td scope="col">= g</td>
                        </tr>
                        <tr>
                            <td scope="col">2. Pintu</td>
                            <td scope="col">= b</td>
                            <td scope="col">5. Body</td>
                            <td scope="col">= e</td>
                            <td scope="col">8. Belt</td>
                            <td scope="col">= h</td>
                        </tr>
                        <tr>
                            <td scope="col">3. Sign</td>
                            <td scope="col">= c</td>
                            <td scope="col">6. Engsel</td>
                            <td scope="col">= f</td>
                            <td scope="col">9. Rangka</td>
                            <td scope="col">= i</td>
                        </tr>
                    </tbody>
                </table>
