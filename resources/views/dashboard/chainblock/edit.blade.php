...
@section('content')
    <div
        ...
        <h1>Edit Data Chain Block</h1>
    </div>
    <form action="{{ route('data-chainblock.update', $chainblock->id) }}" method="POST" class="mb-5 col-lg-12" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="row">
            <div class="mb-3 col-md-6">
                ...
                <input type="text" name="no_chainblock" id="no_chainblock" placeholder="Masukkan No Chain Block"
                    class="form-control @error('no_chainblock') is-invalid @enderror" value="{{ old('no_chainblock') ?? $chainblock->no_chainblock}}" readonly>
                @error('no_chainblock')
            ...
            <div class="mb-3 col-md-6">
                ...
                <select name="location_id" id="location_id" class="form-control @error('location_id') is-invalid @enderror">
                    ...
                    @foreach ($locations as $location)
                        <option value="{{ $location->id }}" {{ old('location_id') ?? $chainblock->location_id == $location->id ? 'selected' : '' }}>
